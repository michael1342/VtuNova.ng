...
const BuyData = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedNetwork || !phone || !selectedPlan || !isVerified) return;
    setLoading(true);
          ...
          <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3 animate-[fadeIn_.3s_ease]">
            ...
            <div>
              ...
              <div className="text-xs text-text-muted mt-0.5">
                {selectedPlan.name} (₦{selectedPlan.price.toLocaleString()}) has been activated successfully on {phone}.
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-4">
            <div className="bg-bg-card border border-border rounded-2xl p-5 space-y-5">
              ...
              {/* Plan Type and Select Data Bundle */}
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {filteredPlans.map(plan => (
                        <option key={plan.id} value={plan.id} className="bg-bg-dark text-text-white">
              ...
              <div className="bg-bg-card border border-border rounded-2xl p-5 space-y-3">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-text-white font-semibold">{selectedPlan.name}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-text-muted">Amount</span>
                    <span className="text-text-white font-bold text-sm">₦{selectedPlan.price.toLocaleString()}.00</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pb-2">
              <button
                type="submit"
                disabled={loading || !selectedNetwork || !phone || !selectedPlan || !isVerified}
